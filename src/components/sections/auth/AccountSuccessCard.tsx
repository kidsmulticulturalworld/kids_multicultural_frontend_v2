...
export default function AccountSuccessCard() {
  ...
  // Adjust content based on where the user came from
  const isFromShop = from === "shop";

  const subtitle = isFromShop
    ? "Now you can continue by making payment and complete your checkout"
    : "Now you can continue by making payment and complete your enrollment";

  const buttonText = isFromShop ? "View Order Confirmation" : "Proceed to Payment";

  const destination = isFromShop ? "/order-confirmation" : "/class-payment";
  ...
  return (
    <div className="w-full max-w-[520px] bg-white rounded-2xl shadow-md p-5 sm:p-6 md:p-10 text-center">
      ...
      <p className="text-center text-sm text-gray-500 mb-7 sm:mb-8 max-w-[320px] mx-auto">
        {subtitle}
      </p>
      ...
      <button
        ...
        className="w-full inline-flex items-center justify-center gap-2 bg-[#3491E8] hover:bg-[#2b7ed0] text-white text-sm font-semibold py-3.5 rounded-xl transition-colors cursor-pointer"
      >
        {buttonText}
        <Image
